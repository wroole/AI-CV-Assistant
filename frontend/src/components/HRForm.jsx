...
export default function HRForm() {
  ...
  const submit=useCallback(async e=>{e.preventDefault();if(!file){setError('Choose a candidate PDF first.');return;}if(source==='text'&&!jdText.trim()){setError('Add the job description text first.');return;}if(source==='url'&&!jdUrl.trim()){setError('Add a public job URL first.');return;}if(source==='pdf'&&!jdFile){setError('Choose a job description PDF first.');return;}setLoading(true);setError('');setResult(null);const body=new FormData();body.append('file',file);body.append('provider',provider);if(source==='text')body.append('job_description',jdText);if(source==='url')body.append('job_description_url',jdUrl);if(source==='pdf')body.append('job_description_file',jdFile);try{setResult(await apiResult(await fetch(API_URL,{method:'POST',body})));}catch(err){setError(err.message);}finally{setLoading(false);}},[file,source,jdText,jdUrl,jdFile,provider]);
